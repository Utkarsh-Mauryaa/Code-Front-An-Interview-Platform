  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          variant="ghost"
          className="w-full h-9 rounded-xl text-xs font-medium text-slate-400 hover:text-white hover:bg-white/5 border border-white/8 hover:border-white/15 transition-all duration-200 gap-2"
        >
          <MessageSquareIcon className="h-3.5 w-3.5" />
          {existingComments.length > 0 ? `${existingComments.length} Comment${existingComments.length !== 1 ? "s" : ""}` : "Add Comment"}
        </Button>
      </DialogTrigger>

      <DialogContent className="sm:max-w-[580px] bg-[#13131e] border-white/10 rounded-2xl text-white">
                    ...
                    {existingComments.map((c, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="rounded-xl border border-white/8 bg-[#0a0a0f] p-4 space-y-3"
                        >
                          <div className="flex items-center justify-between">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                className="bg-[#0a0a0f] border-white/10 text-white placeholder:text-slate-600 rounded-xl h-28 focus:border-amber-400/50 focus:ring-amber-400/20 transition-colors resize-none text-sm"
              />
            </div>
          </div>
        </div>
