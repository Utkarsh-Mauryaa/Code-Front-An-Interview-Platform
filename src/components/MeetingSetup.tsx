  ...
  return (
    <div className="h-[calc(100vh-70px)] bg-[#0a0a0f] flex items-center justify-center p-6">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/3 w-64 h-64 bg-blue-500/3 rounded-full blur-3xl" />
      </div>
      ...
      <div className="w-full max-w-[1100px] mx-auto relative">
        ...
        >
          ...
          >
            ...
            <div className="flex-1 min-h-[360px] rounded-xl overflow-hidden bg-[#0d0d14] border border-white/8 relative">
              <style>{`
              ...
              {disableCamera && (
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-[#0d0d14]">
                  <div className="p-3 rounded-2xl bg-white/5 border border-white/8">
                    <VideoOffIcon className="h-6 w-6 text-slate-500" />
                  </div>
          ...
          >
            <div className="mb-6">
              ...
              <p className="text-xs text-slate-500 mt-0.5 break-all font-mono">{call.id}</p>
            </div>

            <div className="flex-1 flex flex-col justify-between">
              <div className="space-y-2">
                {/* Camera Toggle */}
                <div className="flex items-center justify-between p-4 rounded-xl bg-[#0a0a0f] border border-white/8 hover:border-white/12 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center transition-colors ${
                      !disableCamera ? "bg-amber-400/10" : "bg-white/5"
                    }`}>
                      <CameraIcon className={`h-4 w-4 transition-colors ${!disableCamera ? "text-amber-400" : "text-slate-500"}`} />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-white">Camera</p>
                      <p className="text-xs text-slate-500">{disableCamera ? "Off" : "On"}</p>
                    </div>
                  </div>
                  <Switch
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-[#0a0a0f] border border-white/8 hover:border-white/12 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`h-9 w-9 rounded-xl flex items-center justify-center transition-colors ${
                      !disableMic ? "bg-amber-400/10" : "bg-white/5"
                    }`}>
                      <MicIcon className={`h-4 w-4 transition-colors ${!disableMic ? "text-amber-400" : "text-slate-500"}`} />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-white">Microphone</p>
                      <p className="text-xs text-slate-500">{disableMic ? "Off" : "On"}</p>
                    </div>
                  </div>
                  <Switch
                    checked={!disableMic}
                ...
                <div className="flex items-center justify-between p-4 rounded-xl bg-[#0a0a0f] border border-white/8 hover:border-white/12 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">Configure devices</p>
                    </div>
                  </div>
                  <DeviceSettings />
                </div>
              ...
              <div className="space-y-3 mt-6">
                <Button
                  className="w-full h-11 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-semibold transition-all duration-200 shadow-lg shadow-amber-400/20"
                  onClick={handleJoin}
                >
                  Join Meeting
                </Button>
