...
function MeetingSetup({ onSetupComplete }: { onSetupComplete: () => void }) {
    ...
    const call = useCall(); // we can use this hook because we have already passed call in the parent component
    if (!call) {
        return null;
    }
    useEffect(() => {
    ...
    }, [disableCamera, call.camera]); // call.camera: It contains pre .status (e.operties likg., 'enabled', 'disabled'), the current .deviceId, and the actual video track.

    useEffect(() => {
        if (disableMic) call.microphone.disable();
        else call.microphone.enable();
    }, [disableMic, call.microphone]); // call.microphone: It contains properties like .status (e.g., 'enabled', 'disabled'), the current .deviceId, and the actual audio track.
            ...
            <div className="w-full max-w-[1200px] mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card className="md:col-span-1 p-6 flex flex-col">
                        ...
                        <div className="mt-4 flex-1 min-h-[400px] rounded-[10px] overflow-hidden bg-muted/50 border relative">
